...
export  function projectCard(item){
   return `<div class="project-item outline-0 shadow-lg rounded-xl px-6 py-4 bg-gray-200  mx-auto">
       ...
       <p class="badge"> ${item.category}</p></div>
        ...
        <span class="pt-2  block text-left text-gray-600 text-sm">${item.createdAt}</span>
        <a class="text-left block text-gray-600 pt-2 text-sm hover:text-gray-400" href=${item.link} target="_blank" rel="noopener noreferrer">${item.link}</a>
      </div>`;
}
